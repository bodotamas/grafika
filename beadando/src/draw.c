...
extern int flare_active;
extern float flare_alpha;
extern float flare_scale;

#define MAX_STARS 50                
extern int stars_active;
extern struct 
{
    float x, y, z;
} stars[MAX_STARS];
...
void draw_environment(World world, Rotate* rotate, Move move )
{
    ...
    if (flare_active) 
    {
        glPushMatrix();
        glTranslatef(0, 0, 0); 
        glScalef(flare_scale, flare_scale, flare_scale);
        glColor4f(1.0f, 0.7f, 0.2f, flare_alpha); 
